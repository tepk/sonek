...
function locatePluginDir(name, configPath, appPath) {
  ...
  var globalPath = _resolveFrom2.default.silent((0, _path.resolve)(_globalModules2.default, '..'), name);
  if (globalPath) {
...
function loadPlugins(plugins) {
  plugins.map(function (plugin) {
    try {
      ...
      var _module = require(plugin.path);
      var name = _module.name || plugin.name;

      return { name: name, module: _module };
    } catch (e) {
      console.log(_chalk2.default.red('Unable to load plugin ' + plugin.name));
...
function isDirectoryMupPlugin(_ref) {
  var name = _ref.name,
