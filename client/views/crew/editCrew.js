Template.editCrew.onCreated(function () {
...
Template.editCrew.onRendered(function () {

})

Template.editCrew.helpers({
    data: function () {
        return Crew.findOne(this._id);
    },
})

Template.editCrew.events({
    "submit #editMember": function (e, t) {
        ...
        var payment = new Date().getFullYear()
            Crew.update(this._id,{
                ...
                globalUI.callback(function (id) {
                    toastr.success('Карточка члена клуба обновлена', 'Выполнено');
                    e.currentTarget.reset()

                })
            )
        Router.go('/viewCrew/' + $(e.currentTarget).attr("member"));
        return false
    },
